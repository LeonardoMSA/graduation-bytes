...
export function SecretConsole({ onBackToRetro }: SecretConsoleProps) {
  ...
  const handleCommand = useCallback(
    (cmd: string) => {
      addLine(`<span style="color:#c8ff00">$</span> ${escapeHtml(cmd)}`);

      if (cmd === "cd") {
        addLine(
          [
            '<span style="color:#00e5ff">Uso:</span> cd &lt;diretório&gt;',
            "",
            "Exemplo:",
            '  <span style="color:#c8ff00">cd luiza</span>',
            '  <span style="color:#c8ff00">cd meninas</span>',
            '  <span style="color:#c8ff00">cd retro</span>',
            "",
      ...
      if (cmd === "cd luiza") {
        addLine('<span style="color:#00e5ff">Abrindo luiza/...</span>');
        window.open("/luiza", "_blank");
        return;
      }

      if (cmd === "cd meninas") {
        addLine('<span style="color:#00e5ff">Abrindo meninas/...</span>');
        window.open("/meninas", "_blank");
        return;
      }

      if (cmd === "cd retro") {
        addLine('<span style="color:#00e5ff">Abrindo retro/...</span>');
        onBackToRetro();
        return;
      }
      ...
      if (cmd.startsWith("cd ")) {
        ...
        addLine(
          `<span style="color:#ff5f57">cd:</span> ${escapeHtml(dir)}: diretório não encontrado`,
        );
        return;
      }
      ...
      const commands: Record<string, () => string | null> = {
        help: () =>
          [
            ...
            '<span style="color:#c8ff00">luiza</span>    — Sobre a aniversariante',
            '<span style="color:#c8ff00">party</span>    — 🎉 Confetes!',
            ...
            '<span style="color:#c8ff00">cd</span>       — Mudar de diretório',
          ].join("<br/>"),
        ...
        luiza: () =>
          [
            '<span style="color:#ff2d7b">Luiza Omena</span> — Bacharela em CC & aniversariante 🎂',
        ...
        ls: () =>
          [
            '<span style="color:#c8ff00">drwxr-xr-x</span>  luiza/        <span style="opacity:0.4">← fotos de lua</span>',
            '<span style="color:#c8ff00">drwxr-xr-x</span>  meninas/   <span style="opacity:0.4">← fotos das garotas</span>',
            '<span style="color:#c8ff00">drwxr-xr-x</span>  retro/        <span style="opacity:0.4">← joga pro retro</span>',
          ].join("<br/>"),
      };
    ...
    <section className="py-24 px-6" style={{ background: "hsl(250,30%,10%)" }}>
      <div className="max-w-3xl mx-auto text-center">
        ...
        <h2 className="font-modern text-3xl sm:text-4xl font-bold mb-2 leading-tight">
          Terminal <span className="text-[#c8ff00]">integrado</span>
        </h2>
        <p className="opacity-50 text-sm mb-8">
          Tente: <code className="text-[#c8ff00]">help</code>
        </p>
